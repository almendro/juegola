...
var Movieclip = {
   ...
   _settings : {
      ...
      'debug' : false,
      'callBack' : null,
      'mode' : 'background', /* nuevo: background || div */
      'div' : '.figura' /* class */
   },
   ...
   _timer : null,
    init : function(options, elmt ) {
        ...
        // comprobamos si se usa el modo tradicional con background o el modo div.figura con una img para escalar
      	trace("mode "+this._settings.mode);
        if(this._settings.mode=='background'){

		      $(this._elmt).css({
		        display : 'block',
		        background : 'url('+this._settings.url+')'
		      });
		    } else if (this._settings.mode=='div'){
		    	if(this._settings.url!='inline'){
    ...
    _delay : function (_this) {
        ...
        $(_this._elmt).each(function(){
		        if(_this._settings.mode=='background'){
    	         $(this).css('background-position', -(_this._cpt*_this._settings.width));
    	       } else if (_this._settings.mode=='div'){
	    	       $(_this._settings.div+' img',this).css({
	    	       	left: -(_this._cpt*_this._settings.width)
	    	       });
    	       }
    ...
    gotoAndStop : function(frame) { // frame : uint between 1 & frametotal
        ...
        if(this._settings.mode=='background'){
    	    ...
    	    $(this._settings.div+' img',this._elmt).css({
    	  	 	left: -(this._cpt*this._settings.width)
    	    });
  	    }	      
